...
extremely important because they define the structure of the dataflow
...
``right cancellation rule for \C{nat} substraction''.}
\begin{lstlisting}
...
\noindent
The general form of the localisation tactical \C{in} is also best
...
goal is a product or a \L+let ... in+, and performs \C{hnf}
otherwise.
...
The \ssr{} \C{case} tactic has a special behaviour on
equalities.\footnote{The primitive \Coq{} behaviour, rewriting right
...
an $\mathcal{L}$-tac expression named \C{ssrautoprop}.

% The combination of the \C{apply} tactic with the discharge tactical
% \C{:} has a special interest, see section \ref{ssec:discharge}.


% \ssr{} also offers an
% extension \C{apply:} of the standard \Coq{} tactic.
% The \ssr{} tactic:
% \begin{lstlisting}
%   apply: H x.
% \end{lstlisting}
% corresponds indeed to the following standard \Coq{} tactic:
% \begin{lstlisting}
%   refine (@H _ ... _ x); try clear H; try clear x.
% \end{lstlisting}
% where the number of possible wildcards to be inserted in
% \C{(@H _ ... _ x)} is guessed using \C{H} and the goal.
% This is specially useful when \C{x} is an implicit parameter of the
% type of \C{H}, whatever its position may be in the list of \C{H}'s
% implicit arguments.
...
\begin{displaymath}
\N{tactic}\ [\N{ident}]
            \C{:}\ \N{d-item}_1\ ...\ \N{d-item}_n\ [\N{clear-switch}]
\end{displaymath}
...
as variables, and not as local definitions. To override this behaviour, 
prefix the name of the local definition with a \C{@},
like in \C{move: @n}.

This is in contrast with the behaviour of the \C{in} tactical (see section
...
much more context to interpret wildcards; in particular it can accomodate
...
The \C{abstract} assigns an abstract constant previosly introduced with
...
Furthermore it is very common to immediately destructure
...
\begin{eqnarray*}
  ...
  \N{i-pattern}& \equiv&
   ...
   &&      \C{[} \N{i-item}^*_1 \;\C{|}\;...\;\C{|}\;\N{i-item}^*_m \C{]}
	   \;|\;\C{-}\;|\;\C{[:}\N{ident}^+\C{]}
\end{eqnarray*}
...
is a view $\N{i-item}$, the view is appplied to the assumption in top
...
\item $[\N{occ-switch}]$\C{->} (resp. $[\N{occ-switch}]$\C{<-}) 
  ...
  and honouring the optional occurrence selector),
  and finally deletes the anonymous fact from the context.
\item\C{[ $\N{i-item}^*_1$ | $...$ | $\N{i-item}^*_m$ ]},
   ...
   subgoals, unless the \C{[$...$]} $\N{i-pattern}$ comes after an initial
...
\item\C{[ $\N{i-item}^*_1$ | $...$ | $\N{i-item}^*_m$ ]}, when it is
...
$\C{(}\N{ipat}\C{,}...\C{,}\N{ipat}\C{)}$ for destructing intro-patterns.

Clears are deferred until the end of the intro pattern. For
example, given the goal \\
\C{0 < x -> (0 < x) \&\& (y < 2)}, the tactic \C{move=>
\{x\} ->} successfully rewrites the goal and deletes \C{x} and the anonymous
equation. If the cleared names are reused in the same intro pattern, a renaming
...
\C{move} should be used for casual decompositions,
such as splitting a pair, and \C{case} should be used for actual destructions,
...
not actually generalised, but rather selected for substitution. Occurrence
