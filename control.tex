...
It is hence recommended practice that the proof of any subgoal should
  ...
  goal}, like \C{discriminate}, \C{contradiction} or \C{assumption}.

\todo{Shall we mention \C{now} and \C{easy?}}
...
proof of $\N{term}$ in the context of \C{G}. The  second generated
subgoal is of the form \C{$\N{term}$ -> G}, where $\N{term}$ becomes
the new top assumption, instead of being introduced with a fresh
name. At the proof-term level, the \C{have} tactic creates a $\beta$
...
\C{/*wlog_statement*/ -> G}. However, such cuts are usually rather
painful to perform by hand, because the statement
\C{/*wlog_statement*/} is tedious to write by hand, and somtimes even
to read.

\ssr{} implements this kind of reasoning step through the \C{without loss}
tactic, whose short name is \C{wlog}. It offers support to describe
...
should be generalized. The general syntax of \C{without loss} is:
\begin{lstlisting}
...
\noindent
If an $\N{ident}$ is prefixed with the \C{@} prefix mark, then a
...
form \C{(x := y)} is just a renaming of \C{y} into \C{x}. In
